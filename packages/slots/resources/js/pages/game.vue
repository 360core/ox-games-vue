<template>
  <div class="d-flex flex-column fill-height py-3">
    ...
    <div v-if="ready" class="d-flex flex-wrap justify-center align-center flex-1">
      <div class="d-flex flex-wrap justify-center align-stretch slotAnimalIner">
        <div class="line-btns left d-flex align-center justify-space-between flex-row pa-5 pa-md-0">
          <v-btn v-for="i in line_cols[0]" :key="`lines-1-` + i" small class="slotBtn pa-0" :disabled="playing"
            :class="{ 'primary': line_blink.indexOf(i) != -1 && !animation.is_line_showig }" type="button"
            @mouseover="lineShow(i)" @mouseout="lineHide(i)" @click="sound('click'); lines = i; linesShow();">
            {{ i }}
          </v-btn>
        </div>
        <div class="d-flex flex-wrap align-center justify-center canvasGameMid">
          <canvas ref="canvas" width="1400" height="600" />
        </div>
        <div class="line-btns right d-flex flex-wrap align-stretch justify-space-between flex-row pa-5 pa-md-0">
          <v-btn v-for="i in line_cols[1]" :key="`lines-2-` + i" small class="slotBtn pa-0" :disabled="playing"
            :class="{ 'primary': line_blink.indexOf(i) != -1 && !animation.is_line_showig }" type="button"
            @mouseover="lineShow(i)" @mouseout="lineHide(i)" @click="sound('click'); lines = i; linesShow();">
            {{ i }}
          </v-btn>
    ...
    <div class="formBottmControllMain">
      <play-controls class="formBottmControll" ref="controls" :loading="loading"
        :disabled="!ready || account.balance < totalBetAmount || totalBetAmount === 0" :playing="playing"
        @bet-change="bet = $event" @play="play">

        <template v-slot:before-bet-input>
          <div class="d-flex align-center inputboxMn lineBxmn">
            <!-- Minus button -->
            <v-btn class="minusBtn" small text icon color="primary"
              @click="() => { lines = Math.max(1, --lines); blurLines(); }">
              <v-icon small>
                mdi-minus
              </v-icon>
            </v-btn>

            <!-- Input field for Lines -->
            <v-text-field type="number" ref="linesInput" v-model.number="lines" :label="$t('Lines')" dense
              :rules="[validationInteger, v => validationMin(v, 1), v => validationMax(v, 20)]" :disabled="playing"
              outlined :full-width="false" class="lines-input text-center">
            </v-text-field>

            <!-- Plus button -->
            <v-btn class="plusBtn" small text icon color="primary"
              @click="() => { lines = Math.min(20, ++lines); blurLines(); }">
              <v-icon small>
                mdi-plus
              </v-icon>
            </v-btn>
        ...
        <template v-slot:after-bet-input>
          <div class="d-flex align-center inputboxMn totalBetBx">
            <v-text-field :label="$t('Total bet')" :value="totalBetAmount" dense :readonly="!playing"
              :disabled="playing || !ready" outlined :full-width="false" class="total-bet-input text-center" />
          </div>
        </template>
...
export default {
  ...
  components: { BlockPreloader, GameMessage, PlayControls,Info, ModalInfo },
  ...
  methods: {
    ...
    async play(bet) {
      ...
      if (this.playing) {
        this.result = {
          ...
          withdrawablebalance: game.account.withdrawablebalance,
    ...
    showResult() {
      ...
      this.updateUserWithdrawalBalance(this.result.withdrawablebalance)
