<template>
  <div class="d-flex flex-column fill-height py-0" style="background-image: url('/images/games/heads-or-tails/background.jpg');background-size: cover;background-position: center;">
    ...
    <play-controls ref="controls" :loading="loading" :playing="playing" :disabled="!formIsValid || !is_ready" @play="play" />
...
export default {
  ...
  methods: {
    ...
    async play (bet) {
      ...
      if (game.win > 0) {
        this.result.sound = this.config.sounds.win || winSound
        this.result.message = game.gameable.result + ', ' + this.$t('you won') + ' ' + game.win
      } else {
        this.result.sound = this.config.sounds.lose || loseSound
