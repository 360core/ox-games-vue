<template>
  <div class="d-flex flex-column fill-height pt-md-3 pt-12">
    <!-- style="background-image: url('/images/game-lucky-wheel-bg.png');background-size: cover;background-position: center;" -->
    ...
    <div v-if="ready" class="d-flex justify-center fill-height align-center">
      <div class="wheel">
        <object>
          <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="2600"
            height="2600" viewBox="0 0 2600 2600">
            <defs>
            ...
            <g :transform="'translate(1300,1300) rotate(' + animation.spinAngle + ')'" clip-path="url(#wheel_clip)">
              <g v-for="(section, idx) in config.sections" :ref="idx" class="section"
                :transform="'rotate(' + (idx * 360 / config.sections.length) + ')'">
                <path class="section"
                  :class="animation.index != null && (animation.stage == 0 || animation.stage >= 4) && animation.index == idx ? 'win' : ''"
                  :d="'M0 00 -' + sectorX.toFixed(2) + ' -' + sectorY.toFixed(2) + ' A1000 1000 0 0 0-' + sectorX.toFixed(2) + ' ' + sectorY.toFixed(2) + 'Z'"
                  :fill="section.backgroundColor" />
                <text class="label" :x="getLabelX(section)" :y="getLabelY(section)" :font-size="getLabelSize(section)"
                  ...
                  <template v-if="typeof (section.title) == 'string' && section.title.split('\n').length > 1">
                    <tspan v-for="line in section.title.split('\n')" :x="getLabelX(section)"
                      :dy="getLabelSpanDY(section)">{{ line }}</tspan>
                  </template>
                  <template v-else>{{ section.title }}</template>
                </text>
              </g>
              <g v-for="(section, idx) in config.sections" class="section"
                :transform="'rotate(' + (idx * 360 / config.sections.length) + ')'">
                <path class="border"
                  :d="'M' + sectorBX.toFixed(2) + ' -' + sectorBY.toFixed(2) + ' -' + sectorBX.toFixed(2) + ' ' + sectorBY.toFixed(2) + ' -' + sectorX.toFixed(2) + ' -' + sectorY.toFixed(2) + 'Z'" />
              </g>
            ...
            <g transform="translate(1300,1300)">
              ...
              <radialGradient v-for="(section, idx) in config.sections" :id="'LOUTER' + idx" :key="idx" cx="0" cy="0"
                r="100" gradientUnits="userSpaceOnUse">
              ...
              <g v-for="idx in config.sections.length * 2 - 1"
                :transform="'rotate(' + (360 / (config.sections.length * 2) * idx).toFixed(2) + ') translate(-1200,0)'">
                <circle cx="0" cy="0" r="40" stroke-width="0" class="led-outer" filter="url(#ledBrightness)"
                  :fill="getLedOuterFill(idx)" />
                <circle cx="0" cy="0" r="100" stroke-width="0" class="led-outer-light" filter="url(#ledBrightness)"
                  :fill="'url(#LOUTER' + ((idx + animation.ledPanelOffset - 1) % config.sections.length + 1) + ')'" />
              </g>
              <g v-for="idx in config.sections.length * 2"
                :transform="'rotate(' + (360 / (config.sections.length * 2) * idx - 360 / (config.sections.length * 2) / 2).toFixed(2) + ') translate(-1200,0)'">
    ...
    <play-controls ref="controls" :loading="loading" :playing="playing" @play="play" />
