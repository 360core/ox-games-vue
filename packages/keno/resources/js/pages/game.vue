<template>
  ...
  <div v-else class="d-flex flex-column fill-height py-3 align-center justify-center">
    ...
    <div class="d-flex flex-row align-center justify-center">
      <v-btn text color="primary" :disabled="playing || selectedNumbers.length == 0"
        @click="selectedNumbers = []; drawnNumbers = []">
        {{ $t('Clear') }}
      </v-btn>
    ...
    <div class="d-flex flex-row align-center justify-center">
      <div class="formBottmControllMain">
        <play-controls class="formBottmControll" ref="controls" :loading="loading" :playing="playing"
          :disabled="!formIsValid || selectedNumbers.length !== selectCount || drawnNumbers.length !== 0 || playing"
          :class="{ loading: !ready }" @play="play" />
      </div>
    </div>
