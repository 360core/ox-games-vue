<template>
  <!-- v-if="landscape" -->
  <div class="d-flex flex-column fill-height py-0 align-center width-100">
    <div class="flex-grow-1 width-100">
      <div class="align-stretch d-flex flex-column game-board justify-center width-100">
        <div class="board-row-1 d-flex flex-grow-1 align-stretch justify-stretch">
          ...
          <div class="align-stretch d-flex flex-column flex-grow-1 justify-stretch middle-cell">
            <div class="align-start animation d-flex flex-grow-1 justify-center">
              <div ref="ThreeJS" class="align-center d-flex dice-animation flex-column justify-start" style="background: url(/images/sic-bo-table-bg.png);background-size: cover;background-position: center;">
        ...
        <div class="board-row-2 d-flex justify-stretch">
          <button v-for="(n, i) in (() => {let r = []; for (i = 4; i <= 17; i++) r.push(i); return r;})()" :key="i" :data-betsize="getBetSize('total', [n])" class="align-center btn btn-total d-flex flex-column pa-2 flex-grow-1" :class="{betted: getBetSize('total', [n]), win: showWin && rollTotal == n}" :disabled="playing" @click="placeBet('total', [n])">
    ...
    <div class="d-flex mt-5 width-100">
      <play-controls ref="controls" :loading="loading" :disabled="!bets.length" :playing="playing" @play="play" @bet-change="b => bet = b">
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      const fontColor = (Color(backColor).rgb().array().reduce((acc, val) => acc + val, 0) / 3) > 127 ? '#333333' : '#000000'
      let d
    ...
    async play (bet, autoPlayInProgress) {
      this.loading = true
      this.playing = true
      this.win = 0
      this.message = null
      ...
      if (autoPlayInProgress && !this.bets.length) {
        this.rebet()
      }
