<template>
  <v-app :class="[navbarVisible ? 'permanent-navbar' : 'temporary-navbar', miniVariant ? 'mini-active' : '']"
    :style="style">
    <system-bar
    ...
    <v-navigation-drawer border="0" v-model="navigationDrawer" :mini-variant.sync="isMobile ? false : miniVariant" app
      ...
      <button v-if="isMobile" @click.stop="navigationDrawer = !navigationDrawer"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="25"
          ...
          class="">
          <g>
            <circle cx="12" cy="12" r="10" fill="#ffffff" opacity="0.3" class=""></circle>
            <path
              d="M15.71 14.29a.996.996 0 0 1-.71 1.7c-.26 0-.51-.1-.71-.29L12 13.41 9.71 15.7c-.2.2-.45.29-.71.29s-.51-.1-.71-.29a.996.996 0 0 1 0-1.41L10.58 12 8.29 9.71A.996.996 0 1 1 9.7 8.3l2.29 2.29 2.29-2.29a.996.996 0 1 1 1.41 1.41L13.4 12l2.29 2.29z"
    ...
    <chat v-if="authenticated && chatEnabled" v-model="chatDrawer" :permanent="!isMobile" :temporary="isMobile"
      @message="setUnreadChatMessagesCount" />
    ...
    <v-app-bar v-else app :clipped-left="!navbarVisible" :color="appBarBackground">
      ...
      <button v-if="!isMobile"
        class="v-app-bar__nav-icon v-btn2 v-btn--icon  max-md:mr-1  ml-0 rounded-lg !justify-center"
        style="width: 35px; height: 35px; background-color: rgba(255, 255, 255, 0.15);" @click="toggleMiniVariant">
        <div class="v-btn__content">
      ...
      <template v-else-if="authenticated">
        <div class="d-flex align-center header-action g-4">
          <game-search-modal v-if="!isMobile">
            <template #default="{ on }">
              <v-btn v-on="on" density="compact" text icon :style="{
    ...
    <v-dialog v-model="showModal" @click.self="closeModal" max-width="50rem" content-class="auth-modal-dialog mx-4">
      <div class="game-frame-2 auth-modal md:flex pa-0 ">
        ...
        <div class="w-full d-flex flex-wrap gap-0 grow">
          <div class="w-full md:w-1/2 relative md:h-full max-md:h-60">
            <button
              class="button button-m w-8 h-8 center bg-white bg-opacity-5 rounded-lg d-flex items-center justify-center grow-0 md:hidden absolute top-4 right-4"
              type="button" @click="closeModal()">
              <svg class="w-4 h-4 fill-white/40" viewBox="0 0 32 32">
                <path
                  d="M6.32321 9.23054L13.0927 16L6.32321 22.7694L9.23051 25.6767L15.9998 18.9074L22.7695 25.6771L25.6768 22.7698L18.9073 16.0004L25.6768 9.23095L22.7695 6.32365L16.0002 13.0929L9.23051 6.32324L6.32321 9.23054Z">
                </path>
              </svg>
            </button>
            <img src="/images/ox-login-bg-mobile.jpg" class="w-full h-full object-cover object-top block md:hidden "
              alt="">
            <img :src="`/images/ox-login-bg.jpg`"
              class="w-full h-full object-cover   absolute top-0 left-0 hidden  md:block" alt="">
            ...
            <div class="absolute bottom-0 left-0 w-full  max-md:hidden">
              <h2 class="text-3xl md:text-4xl font-bold text-center w-full whitespace-nowrap overflow-hidden">Level Up
                the
                Chaos</h2>
              <p class="mt-1 w-full text-center whitespace-nowrap overflow-hidden font-semibold text-base">Sign Up &amp;
                Get
                Welcome Bonus</p>
            </div>
          </div>
          <div class="w-full md:w-1/2 h-full px-6 py-5">
            ...
            <div key="login" v-if="activeTab === 'login'"
              class="form-container grow items-start justify-between h-full">
              <!-- <h2>Login Form</h2>
              ...
              <div class="w-full d-flex items-center justify-between">
                ...
                <button
                  class="button button-m w-8 h-8 center bg-white bg-opacity-5 rounded-lg md:!flex !flex-0 items-center justify-center grow-0 max-md:!hidden"
                  type="button" @click="closeModal()">
                  <svg class="w-4 h-4 fill-white/40" viewBox="0 0 32 32">
                    <path
                      d="M6.32321 9.23054L13.0927 16L6.32321 22.7694L9.23051 25.6767L15.9998 18.9074L22.7695 25.6771L25.6768 22.7698L18.9073 16.0004L25.6768 9.23095L22.7695 6.32365L16.0002 13.0929L9.23051 6.32324L6.32321 9.23054Z">
                ...
                <v-text-field v-model="form.password" :label="$t('Password')"
                  ...
                  name="password" dense class="custom-text-field" :rules="[validationRequired]"
                  :error="form.errors.has('password')" :error-messages="form.errors.get('password')" outlined
                  :counter="true" @click:append="showPassword = !showPassword" @keydown="clearFormErrors" />


                <div class="flex justify-between items-center ">

                  <v-checkbox v-model="form.remember" name="remember" :label="$t('Remember me')" color="primary"
                    true-value="1" false-value="" class="text-sm !pt-0 !mt-[0px]" />
                  <div class="">
                ...
                <v-btn type="submit" color="primary" class="primary w-full"
            ...
            <div key="signup" v-if="activeTab === 'signup'"
              class="form-container grow items-start justify-between h-full">
              <div class="w-full d-flex items-center justify-between">
              ...
              <v-form ref="form" v-model="formIsValid" @submit.prevent="register" class="w-full grow">
                <!-- Name Field -->
                <v-text-field v-model="form.name" :label="$t('Name')" type="text" name="name" dense
                  class="custom-text-field" :rules="[validationRequired]" :error="form.errors.has('name')"
                  :error-messages="form.errors.get('name')" outlined @keydown="clearFormErrors($event, 'name')" />
                ...
                <form-parameter v-for="field in fields" :key="field.id" v-model="form.fields[field.id]"
                  :parameter="field" :form="form" dense form-key="fields" />

                <v-text-field v-model="form.password" :label="$t('Password')"
                  :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'" :type="showPassword ? 'text' : 'password'"
                  name="password" dense class="custom-text-field"
                  :rules="[validationRequired, v => validationMinLength(v, 8)]" :error="form.errors.has('password')"
                  :error-messages="form.errors.get('password')" outlined :counter="true"
                  @click:append="showPassword = !showPassword" @keydown="clearFormErrors($event, 'password')" />
                ...
                <v-text-field v-model="form.password_confirmation" :label="$t('Confirm password')"
                  ...
                  name="password_confirmation" dense class="custom-text-field"
                ...
                <v-checkbox v-model="agreementAccepted" color="primary" class="custom-checkbox mt-3 pt-0 mb-2"
                  hide-details dense>
            ...
            <div key="reset" v-if="activeTab === 'reset'"
              ...
              <v-form ref="form" v-model="formIsValid" @submit.prevent="reset" class="w-full grow">
                <v-text-field v-model="form.email" :label="$t('Email')" type="email" name="email" dense
                  ...
                  class="custom-text-field" :error-messages="form.errors.get('email')" outlined
                  @keydown="clearFormErrors" />

                <vue-recaptcha v-if="recaptchaPublicKey" ref="recaptcha" :sitekey="recaptchaPublicKey"
                  :loadRecaptchaScript="true" :theme="this.$vuetify.theme.isDark ? 'dark' : 'light'"
                  @verify="token => form.recaptcha = token" class="mb-3" />

                <v-btn type="submit" color="primary" class="primary w-full"
                  :disabled="!formIsValid || form.busy || (!!recaptchaPublicKey && !form.recaptcha)"
                  :loading="form.busy">
                  {{ $t('Reset Pasword') }}
                </v-btn>
                <div class="pa-0 gap-2 text-sm font-semibold mt-6 opacity-80">
                  <button type="button" :class="{ active: activeTab === 'login' }" @click="activeTab = 'login'"
                    class="font-weight-medium flex items-center justify-center text-center mx-auto gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                      stroke="currentColor" class="size-6 w-4 h-4">
    ...
    <div v-if="isMobile" :color="appBarBackground"
      class="mobile-bottom-bar w-full !fixed bottom-0 left-0 !flex items-center justify-around bg-[#232625] py-3.5 px-4 z-10">
      <div class="shrink-0">
        <button class="flex flex-col gap-1 items-center" @click.stop="navigationDrawer = !navigationDrawer">
          <div class="v-btn__content">
            <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width="20"
              height="20">
              <path
                d="M5,10.75H7A3.383,3.383,0,0,0,10.75,7V5A3.383,3.383,0,0,0,7,1.25H5A3.383,3.383,0,0,0,1.25,5V7A3.383,3.383,0,0,0,5,10.75ZM2.75,5c0-1.577.673-2.25,2.25-2.25H7c1.577,0,2.25.673,2.25,2.25V7c0,1.577-.673,2.25-2.25,2.25H5C3.423,9.25,2.75,8.577,2.75,7Z"
                fill="currentColor" opacity="0.5" />
              <path
                d="M19,1.25H17A3.383,3.383,0,0,0,13.25,5V7A3.383,3.383,0,0,0,17,10.75h2A3.383,3.383,0,0,0,22.75,7V5A3.383,3.383,0,0,0,19,1.25ZM21.25,7c0,1.577-.673,2.25-2.25,2.25H17c-1.577,0-2.25-.673-2.25-2.25V5c0-1.577.673-2.25,2.25-2.25h2c1.577,0,2.25.673,2.25,2.25Z"
                fill="currentColor" />
              <path
                d="M19,13.25H17A3.383,3.383,0,0,0,13.25,17v2A3.383,3.383,0,0,0,17,22.75h2A3.383,3.383,0,0,0,22.75,19V17A3.383,3.383,0,0,0,19,13.25ZM21.25,19c0,1.577-.673,2.25-2.25,2.25H17c-1.577,0-2.25-.673-2.25-2.25V17c0-1.577.673-2.25,2.25-2.25h2c1.577,0,2.25.673,2.25,2.25Z"
                fill="currentColor" />
              <path
                d="M5,22.75H7A3.383,3.383,0,0,0,10.75,19V17A3.383,3.383,0,0,0,7,13.25H5A3.383,3.383,0,0,0,1.25,17v2A3.383,3.383,0,0,0,5,22.75ZM2.75,17c0-1.577.673-2.25,2.25-2.25H7c1.577,0,2.25.673,2.25,2.25v2c0,1.577-.673,2.25-2.25,2.25H5c-1.577,0-2.25-.673-2.25-2.25Z"
                fill="currentColor" />
            </svg>
          </div>
      ...
      <div class="shrink-0" v-if="authenticated && chatEnabled">
        <button class="flex flex-col gap-1 items-center" v-if="chatEnabled" @click="chatDrawer = !chatDrawer">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
            ...
            <path d="M7.5 13.5c2.5 2.5 6.5 2.5 9 0" stroke-opacity="0.5" />
          </svg>
          ...
          <v-list class="theme-dropdown">
            ...
            <v-list-item>
              <v-list-item-action class="!ml-0 !mr-2.5">
                <v-switch :input-value="settings.sounds" :value="settings.sounds" color="primary"
                  @change="saveSettings('sounds', $event)" class="scale-95" dense />
              </v-list-item-action>
              <span class="text-sm">{{ $t('Sounds') }}</span>
            </v-list-item>
            <v-list-item v-if="gameFeedEnabled">
              <v-list-item-action class="!ml-0 !mr-2.5">
                <v-switch :input-value="isMobile ? false : settings.gameFeed" :value="settings.gameFeed"
                  :disabled="isMobile" color="primary" class="scale-95" @change="saveSettings('gameFeed', $event)" />
              </v-list-item-action>
    ...
    <component :is="footerComponent" v-if="!(isMobile && (isGamePage || isPredictionPage || isProviderGamePage))"
      :inset="navbarVisible" :mini-variant="isMobile ? false : miniVariant" />
  </v-app>
</template>
