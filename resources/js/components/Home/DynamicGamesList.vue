<template>
  <div v-if="games.length" :class="classes">
    ...
    <v-row v-if="filter && categories.length > 1">
      <v-col>
        <v-chip-group v-model="category" active-class="primary" mandatory>
    ...
    <v-row v-if="search" justify="center">
      <v-col md="6" lg="3">
        <v-text-field v-model="input" :label="$t('Search')" solo clearable rounded dense
          prepend-inner-icon="mdi-magnify" />
      </v-col>
    </v-row>

    <div v-if="gamesList.length && !isMobile" ref="games" justify="start" class="game-card-grid">
      <v-col v-for="(game, i) in gamesList" :key="i" class="game-card" :class="cardClasses" :data-name="game.name"
        ...
        <v-hover v-slot="{ hover }">
          <v-card class="display-style-card" :elevation="hover ? 12 : 2">
            <router-link to="markets/crypto-prediction">
              <img src="/images/games/crypto-prediction/thumb.jpg" class="game-banner align-center text-center"
                :class="{ hover }">

              <v-card-title class="text-no-wrap">Crypto Prediction</v-card-title>
              <v-btn fab color="primary" class="play-button" :class="{ hover }">
    ...
    <VueSlickCarousel v-if="isMobile && gamesList.length" :arrows="true" :dots="false" :infinite="true"
      :slidesToShow="3" :slidesToScroll="1" class="game-card mobile-slider -mx-1" :class="cardClasses" :autoplay="true"
      :autoplaySpeed="3000">
